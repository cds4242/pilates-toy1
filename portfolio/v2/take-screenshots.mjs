...
// 회원: 010-9000-1000 ~ 010-9079-1079, password Test1234!
const MEMBER_LOGIN = { phone: "010-9000-1000", password: "Test1234!" };
// 강사: instructor1 / admin1234
...
const captures = [
  ...
  // Instructor (mobile)
  { kind: "mobile", name: "20_instructor_schedule", url: "/instructor/schedule", auth: "instructor", wait: 1500 },
  // Admin (desktop)
...
async function memberLogin(page) {
  ...
  await phone.fill(MEMBER_LOGIN.phone);
  const pw = page.locator('input[id="password"], input[type="password"]').first();
  await pw.fill(MEMBER_LOGIN.password);
...
async function main() {
  ...
  // Group by (kind, auth) to share contexts
  ...
  for (const [key, items] of groups) {
    ...
    for (const cap of items) {
      ...
      console.log(`  ${cap.name} -> ${cap.url}`);
      try {
        await page.goto(`${BASE}${cap.url}`);
        await page.waitForTimeout(cap.wait);
        // Hide the toaster overlay so it doesn't pollute screenshots
        try {
