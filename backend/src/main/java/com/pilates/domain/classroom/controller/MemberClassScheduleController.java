...
/**
 * 수업 시간표 조회 API (회원용).
 * 인증 없이도 조회 가능. 로그인 시 본인 예약 상태 포함.
 */
...
public class MemberClassScheduleController {
    ...
    @Operation(summary = "수업 목록 조회", description = "날짜 범위로 수업 목록 조회. 로그인 시 본인 예약 상태(RESERVED/FULL/NOT_RESERVED) 포함.")
    @ApiResponses({
    ...
    public ApiResponse<List<ClassScheduleResponse>> listByDateRange(
        ...
        // 인증된 사용자면 본인 예약 상태 포함, 아니면 기본 조회
        LoginMember loginMember = (LoginMember) request.getAttribute(LoginMemberArgumentResolver.LOGIN_MEMBER_ATTRIBUTE);
        if (loginMember != null) {
            return ApiResponse.success(
                    classScheduleService.listByDateRangeWithMyStatus(from, to, loginMember.memberId()));
        }
        return ApiResponse.success(classScheduleService.listByDateRange(from, to));
    }

    // TODO [STEP 8 reservation]: 수업 상세에서 본인 예약 여부 표시
