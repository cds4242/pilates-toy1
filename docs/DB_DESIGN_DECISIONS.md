...
## 11. 프로필 사진 저장 정책 (결정: 2026-05-12)

### 저장 방식
- **DB**: `members.profile_image_url` (VARCHAR 500)에 URL만 저장
- **파일**: Cloudflare R2 (S3 호환 오브젝트 스토리지)
- **관계**: 회원 1:1 프로필 사진 (NULL 허용)

### 파일 규격
- 최대 5MB, JPG/PNG/WebP 허용
- 서버에서 리사이즈: 200x200 (목록 썸네일), 500x500 (상세 표시)
- 파일명 규칙: `{member_public_id}/{timestamp}_{size}.{ext}`
  - 예: `550e8400e29b41d4a716446655440000/1715500800_200.webp`

### Fallback 정책
- `profile_image_url`이 NULL이면 클라이언트에서 이니셜 아바타 표시
- 이니셜: 이름 첫 글자 (복호화 후)
- 배경색: public_id 해시 기반으로 파스텔 색상 결정 (클라이언트 로직)

### 인덱스
- 불필요: 프로필 사진 URL을 WHERE 조건으로 사용하는 쿼리 없음

### 부적절한 이미지 관리
- v1: 관리자가 수동 확인·삭제
- v2: 신고 기능 + AI 기반 필터링 고려

---

## 12. 회색 지대 기록 (STEP 4 전에 결정)
